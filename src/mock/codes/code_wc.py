...
class WordCloud(object):
    ...
    def generate_from_text(self, text):
        ...
        # 词云评测 e485c73f5dd07c29 开始
        import json
        import hashlib

        # 计算mask的md5，用于校验mask
        md5 = hashlib.md5()
        md5.update(str(self.mask).encode('u8'))
        mask = md5.hexdigest()

        # 计算 max_font_size = min_font_size + (n - 1) * font_step
        max_font_size = self.min_font_size + (len(words) - 1) * self.font_step
        ...
        print(json.dumps(review_data, ensure_ascii=False))  # 词云测评 结束
